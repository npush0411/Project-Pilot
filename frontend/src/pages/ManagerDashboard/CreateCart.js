...
const CreateCart = () => {
  ...
  useEffect(() => {
    const fetchMidOrders = async () => {
      try {
        const token = localStorage.getItem('token');
        const res = await axios.get(`${BASE_URL}/get-requirement-details`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
        if (res.data.success) {
          setMidOrders(res.data.midOrders);
        } else {
          console.error('Failed to fetch mid orders');
        }
      } catch (err) {
        console.error('Error fetching mid orders:', err);
      }
    };
