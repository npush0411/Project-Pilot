...
const Distribute = () => {
  ...
  const [assignStatus, setAssignStatus] = useState(null); // null | 'loading' | 'success' | 'rejected' | 'error'
  ...
  useEffect(() => {
    ...
    const match = projects.filter(p => (p.slot?.slotn === slot || p.slot?.slotn === -1) && new Date(p.slot.date).toDateString() === today && p.ack !== 1);
  ...
  <div className="distribute-container">
    ...
    <div className="marginn">
      ...
      {importedAndFiltered.length === 0 ? (
        <p>No projects found for this slot.</p>
      ) : (
