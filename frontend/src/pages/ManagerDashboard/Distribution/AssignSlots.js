...
const AssignSlots = () => {
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      try {
        ...
        const liveProjects = res.data.data.filter((p) => !p.isCompleted);
  ...
  const handleAssignSlot = async () => {
    if (!selectedSlot || !selectedDate || selectedProjects.length === 0) {
  ...
  return (
    <div className="assign-container">
      ...
      <button className="assign-btn" onClick={handleAssignSlot} disabled={loading}>
